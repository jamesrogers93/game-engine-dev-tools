...
namespace DAE2JMP
{
    ...
    bool DAEImporterSkin::import(const COLLADAFW::SkinControllerData* skin)
    {
        ...
        DAESkin skinData = getSkinData(skin);
        
        return this->mDAEImporter->addLoadedSkin(skin->getObjectId(), skinData);
    }

    DAESkin DAEImporterSkin::getSkinData(const COLLADAFW::SkinControllerData* skinControllerData)
    {
        DAESkin skinData;  // Contains the accumalated skinVertex data
